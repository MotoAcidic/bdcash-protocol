...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sAPOLLONPercentage, QString& szAPOLLONPercentage)
{
    int nPrecision = 2;
    ...
    szAPOLLONPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sAPOLLONPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelAPOLLONPercent->setText(sPercentage);
    // ui->labelzAPOLLONPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zAPOLLON.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    ...
    ui->labelLockedBalance->setVisible(showAPOLLONLocked || showWatchOnlyAPOLLONLocked);
    ui->labelWatchLocked->setVisible(showWatchOnlyAPOLLONLocked && showWatchOnly);

    // zAPOLLON
    bool showzAPOLLONAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzAPOLLONUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzAPOLLONImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    // ui->labelzBalanceMature->setVisible(showzAPOLLONAvailable);
    // ui->labelzBalanceMatureText->setVisible(showzAPOLLONAvailable);
    // ui->labelzBalanceUnconfirmed->setVisible(showzAPOLLONUnconfirmed);
    // ui->labelzBalanceUnconfirmedText->setVisible(showzAPOLLONUnconfirmed);
    // ui->labelzBalanceImmature->setVisible(showzAPOLLONImmature);
    // ui->labelzBalanceImmatureText->setVisible(showzAPOLLONImmature);

    // Percent split
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelAPOLLONPercent->setVisible(showPercentages);
    // ui->labelzAPOLLONPercent->setVisible(showPercentages);
