    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zAPOLLON.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAPOLLON seed used to mint this zAPOLLON is not the same as currently hold in the wallet";
            else
