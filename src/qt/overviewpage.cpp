...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeBDCASHAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBDCASHAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBDCASHAmount = strAnonymizeBDCASHAmount.remove(strAnonymizeBDCASHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBDCASHAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBDCASHAmount * COIN) nMaxToAnonymize = nAnonymizeBDCASHAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBDCASHAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBDCASHAmount));
        strAnonymizeBDCASHAmount = strAnonymizeBDCASHAmount.remove(strAnonymizeBDCASHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBDCASHAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
