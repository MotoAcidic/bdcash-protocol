...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter an BDCASH address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no BDCASH: URI
...
{
    // Convert bdcash:// to bdcash:
    //
    //    Cannot handle this later, because bdcash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bdcash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BDCASH.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BDCASH.lnk
...
{
    return GetAutostartDir() / "bdcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bdcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BDCASH\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bdcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bdcash app to startup item list
