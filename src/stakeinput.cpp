...
// The zAPOLLON block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zAPOLLON is a hash of the serial
...
{
    //Create an output returning the zAPOLLON that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPIVOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAPOLLON output", __func__);
...
{
    //The unique identifier for a APOLLON stake is the outpoint
