                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zAPOLLON ought to be enough for anybody." - Bill Gates, 2017
    ui->zAPOLLONpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAPOLLONSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAPOLLONpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzAPOLLON_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAPOLLON is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zAPOLLON, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAPOLLON...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAPOLLON in ") +
...
void PrivacyDialog::on_pushButtonSpendzAPOLLON_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAPOLLON is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zAPOLLON, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zAPOLLON
        sendzAPOLLON();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAPOLLON
    sendzAPOLLON();
}
...
void PrivacyDialog::sendzAPOLLON()
{
    QSettings settings;
    ...
    double dAmount = ui->zAPOLLONpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAPOLLON is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAPOLLON</b>";
    ...
    // use mints from zAPOLLON selector if applicable
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zAPOLLON transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zAPOLLON was spent successfully update the addressbook with the label
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAPOLLON Spend #: ") + QString::number(nCount) + ", ";
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zAPOLLONpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAPOLLON </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAPOLLON "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAPOLLON "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zAPOLLON "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAPOLLON </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAPOLLON </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zAPOLLON </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzAPOLLONSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzAPOLLON->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zAPOLLON
        //ui->pushButtonMintzAPOLLON->setEnabled(false);
        //ui->pushButtonMintzAPOLLON->setToolTip(tr("zAPOLLON is currently disabled due to maintenance."));

        // Spend zAPOLLON
        ui->pushButtonSpendzAPOLLON->setEnabled(false);
        ui->pushButtonSpendzAPOLLON->setToolTip(tr("zAPOLLON is currently disabled due to maintenance."));
        ...
        ui->pushButtonSpendzAPOLLON->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
